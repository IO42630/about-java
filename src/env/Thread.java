...
thread synchronization		Every Java object has a single monitor. The monitor can be owned by one thread.
				To access an object a thread needs to wait in the queue until the first thread releases its monitor.
thread status			NEW: 		The thread is created but has not been processed yet.
